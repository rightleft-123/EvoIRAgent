...
def crop_to_smallest(matrix1, matrix2):  
    # 获取两个矩阵的高度和宽度  
    h1, w1 = matrix1.shape[-2], matrix1.shape[-1]  
    h2, w2 = matrix2.shape[-2], matrix2.shape[-1]  
    ...
    # 对第一个矩阵进行裁剪  
    matrix1_cropped = matrix1[..., :min_h, :min_w]  
    
    # 对第二个矩阵进行裁剪  
    matrix2_cropped = matrix2[..., :min_h, :min_w]  
    
    return matrix1_cropped, matrix2_cropped
